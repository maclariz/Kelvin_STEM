...
def make_lettercolkey_from_L2_cluster(
    ...
    colors: str
        A str in ["RGBwheel","cmap"]
        If RGBwheel is used, then colors are generated by a simple hsv_to_rgb routine
        encoding angle as hue
        If cmap is used, then a cmap must be provided as an input and the angular range is
        mapped onto the cmap
    cmap: matplotlib colormap
        This must be a valid colormap, not a colormap name.
        e.g. matplotlib.colormaps['viridis'] or one you generate yourself using
        LinearSegmentedColormap.from_list(cmap_name, color_list) or similar
    Returns
    -------
    lettercolkey: dict
    ...
    for letter in L2key:
        ...
        if colors == "RGBwheel":
            ...
            col = np.array(cmap(h))[:3]
        else:
            print("Please enter either RGBwheel or cmap")
